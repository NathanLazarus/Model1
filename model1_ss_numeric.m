function [KSTAR,CSTAR,LSTAR,WSTAR,RSTAR]=model1_ss_numeric(DELTA,ALFA,BETTA,G,P,ETA,GAMA,ZSTAR)
...
ss3 = GAMA * L^ETA * C - (1/P) * (1-ALFA) * ZSTAR * K^ALFA * L^(-ALFA);
...
while length(SOL.L)<1
    ...
    ss3 = GAMA * L^ETA * C - (1/p_alt) * (1-ALFA) * ZSTAR * K^ALFA * L^(-ALFA);
    [SOL]=solve(ss1,ss2,ss3,C,K,L,'Real',true)
end
