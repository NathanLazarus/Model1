...
for iteration in range(50):
    ...
    print(l(DM.ones(2)*3,DM.ones(2)*3))

    def transformede(rho,capital,zeta):
    ...
    def logquad(capital,zeta,n):
        samplepoints,sampleweights = h_roots(n)
        return (transformede(np.exp(np.sqrt(2)*samplepoints),capital,zeta)/np.sqrt(np.pi))@sampleweights
    ...
    def condition2(consumption,labour,capital,mu):
        temp = (logquad(capital[:T],zeta[:T],20)
             - 1/consumption[:T] - mu)
    ...
    lower_bound_C = vertcat(DM.ones(T)*0.0001, ssc)    # lower bound on the consumption -> not binding anyway
    ...
    upper_bound_L = vertcat(DM.ones(T)*Lmax - 1e-9, ssl)
    ...
    x_0 = vertcat(DM.ones(T+1), DM.ones(T+1),DM.ones(T+1))
    ...
    r = (1/P)*(alpha)*k**(alpha-1)*l**(1-alpha)-delta
    ...
    def _vander_nd(vander_fs, points, degrees):
        ...
        print('************')
        print(points)
        print(functools.reduce(operator.mul, vander_arrays))
    ...
    def l(k,z):
        return _vander_nd_flat_SYM((hermevander_casadiSYM,hermevander_casadiSYM),[k,z],[degree,degree]) @ l_coefs.T
    ...
    print(l_coefs)
    print(c_coefs)
    print(l(DM.ones(5)*ssk,DM.ones(5)))
    print(c(DM.ones(2)*ssk,DM.ones(2)))
    altk = k[133]
T = 2
zeta = DM.ones(3)
print(FOCs(c(DM.ones(3)*ssk,DM.ones(3)),l(DM.ones(3)*ssk,DM.ones(3)),DM.ones(3)*ssk,DM.zeros(2)))
print(altk)
print(altl,altc)
print(FOCs(c(DM.ones(3)*altk,DM.ones(3)),l(DM.ones(3)*altk,DM.ones(3)),DM.ones(3)*altk,DM.zeros(2)))
print('!!!!!!!!!!!!!!!!!')
print(ssk)
print(ssl)
print(ssc)
